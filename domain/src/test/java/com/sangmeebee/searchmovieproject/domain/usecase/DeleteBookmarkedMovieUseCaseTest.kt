...
class DeleteBookmarkedMovieUseCaseTest {
    @Test
    fun `레포지토리에 즐겨찾는 영화 정보 저장을 요청한다`() = runTest {
        ...
        val deleteBookmarkedMovieUseCase = DeleteBookmarkedMovieUseCase(movieRepository)
        // when
        deleteBookmarkedMovieUseCase(movie)
        // then
        coVerify { movieRepository.deleteBookmarkedMovie(movie) }
    }
}
